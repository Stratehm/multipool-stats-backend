 ...
 */
package strat.mining.multipool.stats.client.mvp.model.coinshift;
...
public class CoinshiftDataManager {
	...
	private CoinshiftDataContainer dataContainer;
	...
	public CoinshiftDataManager(ClientFactory clientFactory) {
		this.clientFactory = clientFactory;
		dataContainer = new CoinshiftDataContainer();
	...
	public void loadAllGlobalStats() {
		clientFactory.getCoinshiftServices().getAllGlobalStats(new AsyncCallback<List<GlobalStatsDTO>>() {
	...
	public void loadLastGlobalStats() {
		clientFactory.getCoinshiftServices().getLastGlobalStats(new AsyncCallback<GlobalStatsDTO>() {
			public void onFailure(Throwable caught) {
	...
	private void loadPaidout(final String address, final int requestId) {
		if (address != null) {
			clientFactory.getCoinshiftServices().getAllAddressPaidout(address, new AsyncCallback<List<AddressPaidoutDTO>>() {
	...
	private void loadAllAddressesStats(final String address, final int requestId) {
		if (address != null) {
			clientFactory.getCoinshiftServices().getAllAddressStats(address, new AsyncCallback<List<AddressStatsDTO>>() {
	...
	private void loadLastAddressStats(final String address, final int requestId) {
		if (address != null && dataContainer.getAddresses() != null && dataContainer.getAddresses().size() > 0) {
			clientFactory.getCoinshiftServices().getLastAddressStats(address, new AsyncCallback<AddressStatsDTO>() {
				public void onFailure(Throwable caught) {
		...
		for (GlobalStatsDTO stats : globalStats) {
			...
			if (firstPayoutDetected && !isFirstPayout) {
				...
				if (previousGlobalStats.getTotalBalance() > stats.getTotalBalance() || index + 1 >= globalStats.size()) {
					...
					float totalBTC = previousGlobalStats.getTotalBalance() + previousGlobalStats.getTotalUnexchangedBalance()
							- stats.getTotalBalance() - stats.getTotalUnexchangedBalance();
	...
	public CoinshiftDataContainer getDataContainer() {
