...
import strat.mining.multipool.stats.client.mvp.view.waffle.PaidoutView;
import strat.mining.multipool.stats.client.mvp.view.waffle.impl.PaidoutViewImpl;
...
public class PaidoutViewPresenter implements PaidoutView.PaidoutViewPresenter {
	...
	private void loadData(boolean isAllAddress) {
		...
		// available.
		Map<String, List<AddressPaidoutDTO>> addressesPaidout = clientFactory.getMainDataManager().getWaffleDataManager().getDataContainer()
				.getAddressesPaidout();
		if (addressesPaidout.keySet().containsAll(requestedAddresses)) {
			...
			clientFactory.getMainDataManager().getWaffleDataManager().loadAllPaidout();
		}
	}
