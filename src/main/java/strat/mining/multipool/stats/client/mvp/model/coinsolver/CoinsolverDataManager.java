 ...
 */
package strat.mining.multipool.stats.client.mvp.model.coinsolver;
...
public class CoinsolverDataManager {
	...
	private CoinsolverDataContainer dataContainer;
	...
	public CoinsolverDataManager(ClientFactory clientFactory) {
		this.clientFactory = clientFactory;
		dataContainer = new CoinsolverDataContainer();
	...
	public void loadAllGlobalStats() {
		clientFactory.getCoinsolverServices().getAllGlobalStats(new AsyncCallback<List<GlobalStatsDTO>>() {
	...
	public void loadLastGlobalStats() {
		clientFactory.getCoinsolverServices().getLastGlobalStats(new AsyncCallback<GlobalStatsDTO>() {
			public void onFailure(Throwable caught) {
	...
	private void loadPaidout(final String address, final int requestId) {
		if (address != null) {
			clientFactory.getCoinsolverServices().getAllAddressPaidout(address, new AsyncCallback<List<AddressPaidoutDTO>>() {
	...
	private void loadAllAddressesStats(final String address, final int requestId) {
		if (address != null) {
			clientFactory.getCoinsolverServices().getAllAddressStats(address, new AsyncCallback<List<AddressStatsDTO>>() {
	...
	private void computeAllTotalStats() {
		...
		for (Map<Date, AddressStatsDTO> singleAddressMap : dtoByDateAndAddress.values()) {
			for (Entry<Date, AddressStatsDTO> entry : singleAddressMap.entrySet()) {
				...
				totalStatsForTheGivenDate.setHashrate((totalStatsForTheGivenDate.getHashrate() == null ? 0 : totalStatsForTheGivenDate.getHashrate())
						+ (singleStatsForTheGivenDate.getHashrate() == null ? 0 : singleStatsForTheGivenDate.getHashrate()));
	...
	private void loadLastAddressStats(final String address, final int requestId) {
		if (address != null && dataContainer.getAddresses() != null && dataContainer.getAddresses().size() > 0) {
			clientFactory.getCoinsolverServices().getLastAddressStats(address, new AsyncCallback<AddressStatsDTO>() {
				public void onFailure(Throwable caught) {
	...
	private void computeLastTotalStats() {
		...
		for (List<AddressStatsDTO> addressStats : dataContainer.getAddressesStats().values()) {
			...
			totalStats.setHashrate((totalStats.getHashrate() == null ? 0 : totalStats.getHashrate())
					+ (lastAddressStats.getHashrate() == null ? 0 : lastAddressStats.getHashrate()));
		...
		for (GlobalStatsDTO stats : globalStats) {
			...
			if (firstPayoutDetected && !isFirstPayout) {
				...
				if (previousGlobalStats.getTotalBalance() > stats.getTotalBalance() || index + 1 >= globalStats.size()) {
					...
					float totalBTC = previousGlobalStats.getTotalBalance() + previousGlobalStats.getTotalUnexchanged() - stats.getTotalBalance()
							- stats.getTotalUnexchanged();
				...
				totalMHSinceLastPayout += stats.getTotalMegahashesPerSecond();
				nbStats++;
	...
	public CoinsolverDataContainer getDataContainer() {
