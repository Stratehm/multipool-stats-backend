...
public class WaffleDataManager {
	...
	private WaffleDataContainer dataContainer;
	...
	public WaffleDataManager(ClientFactory clientFactory) {
		this.clientFactory = clientFactory;
		dataContainer = new WaffleDataContainer();
	...
	public void loadAllGlobalStats() {
		clientFactory.getWaffleServices().getAllGlobalStats(new AsyncCallback<List<GlobalStatsDTO>>() {
	...
	public void loadLastGlobalStats() {
		clientFactory.getWaffleServices().getLastGlobalStats(new AsyncCallback<GlobalStatsDTO>() {
			public void onFailure(Throwable caught) {
	...
	private void loadPaidout(final String address, final int requestId) {
		if (address != null) {
			clientFactory.getWaffleServices().getAllAddressPaidout(address, new AsyncCallback<List<AddressPaidoutDTO>>() {
	...
	private void loadAllAddressesStats(final String address, final int requestId) {
		if (address != null) {
			clientFactory.getWaffleServices().getAllAddressStats(address, new AsyncCallback<List<AddressStatsDTO>>() {
	...
	private void loadLastAddressStats(final String address, final int requestId) {
		if (address != null && dataContainer.getAddresses() != null && dataContainer.getAddresses().size() > 0) {
			clientFactory.getWaffleServices().getLastAddressStats(address, new AsyncCallback<AddressStatsDTO>() {
				public void onFailure(Throwable caught) {
	...
	public WaffleDataContainer getDataContainer() {
