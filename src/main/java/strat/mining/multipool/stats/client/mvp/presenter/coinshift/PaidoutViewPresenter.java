...
import strat.mining.multipool.stats.client.mvp.view.coinshift.PaidoutView;
import strat.mining.multipool.stats.client.mvp.view.coinshift.impl.PaidoutViewImpl;
...
public class PaidoutViewPresenter implements PaidoutView.PaidoutViewPresenter {
	...
	private void loadData(boolean isAllAddress) {
		...
		// available.
		Map<String, List<AddressPaidoutDTO>> addressesPaidout = clientFactory.getMainDataManager().getCoinshiftDataManager().getDataContainer()
				.getAddressesPaidout();
		if (addressesPaidout.keySet().containsAll(requestedAddresses)) {
			...
			clientFactory.getMainDataManager().getCoinshiftDataManager().loadAllPaidout();
		}
	}
