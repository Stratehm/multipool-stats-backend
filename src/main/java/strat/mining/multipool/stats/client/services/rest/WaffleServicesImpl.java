...
public class WaffleServicesImpl implements WaffleServices {

	private ClientFactory clientFactory;

	private String serviceBaseUrl = GWT.getHostPageBaseURL() + "services/";

	public WaffleServicesImpl(final ClientFactory clientFactory) {
		this.clientFactory = clientFactory;
	}

	@Override
	public void getAllGlobalStats(final AsyncCallback<List<GlobalStatsDTO>> callback) {
		RequestBuilder builder = new RequestBuilder(RequestBuilder.GET, serviceBaseUrl + "waffle/stats/global/all");
			...
			builder.sendRequest(null, new RequestCallback() {
				public void onResponseReceived(Request request, Response response) {
					if (response.getText() != null && !"".equals(response.getText().trim()) && response.getStatusCode() == 200) {
						List<GlobalStatsDTO> result = clientFactory.getDtoFactory().parseJsonToWaffleGlobalStatsList(response.getText());
	...
	public void getLastGlobalStats(final AsyncCallback<GlobalStatsDTO> callback) {
		RequestBuilder builder = new RequestBuilder(RequestBuilder.GET, serviceBaseUrl + "waffle/stats/global/last");
		builder.setHeader("Accept", "application/json");
		builder.setHeader("Content-Type", "application/json");
		...
		try {
			builder.sendRequest(null, new RequestCallback() {
				public void onResponseReceived(Request request, Response response) {
					if (response.getText() != null && !"".equals(response.getText().trim()) && response.getStatusCode() == 200) {
						GlobalStatsDTO result = clientFactory.getDtoFactory().parseJsonToWaffleGlobalStats(response.getText());
						callback.onSuccess(result);
					} else {
						callback.onFailure(new Exception("Response with error code " + response.getStatusCode() + "\n" + response.getText()));
					}
	...
	public void getLastAddressStats(String address, final AsyncCallback<AddressStatsDTO> callback) {
		RequestBuilder builder = new RequestBuilder(RequestBuilder.GET, serviceBaseUrl + "waffle/stats/" + address + "/last");
		builder.setHeader("Accept", "application/json");
		builder.setHeader("Content-Type", "application/json");
		...
		try {
			builder.sendRequest(null, new RequestCallback() {
				public void onResponseReceived(Request request, Response response) {
					if (response.getText() != null && !"".equals(response.getText().trim()) && response.getStatusCode() == 200) {
						AddressStatsDTO result = clientFactory.getDtoFactory().parseJsonToWaffleAddressStats(response.getText());
						callback.onSuccess(result);
					} else {
						callback.onFailure(new Exception("Response with error code " + response.getStatusCode() + "\n" + response.getText()));
					}
	...
	public void getAllAddressStats(String address, final AsyncCallback<List<AddressStatsDTO>> callback) {
		RequestBuilder builder = new RequestBuilder(RequestBuilder.GET, serviceBaseUrl + "waffle/stats/" + address + "/all");
		builder.setHeader("Accept", "application/json");
		builder.setHeader("Content-Type", "application/json");
		...
		try {
			builder.sendRequest(null, new RequestCallback() {
				public void onResponseReceived(Request request, Response response) {
					if (response.getText() != null && !"".equals(response.getText().trim()) && response.getStatusCode() == 200) {
						List<AddressStatsDTO> result = clientFactory.getDtoFactory().parseJsonToWaffleAddressStatsList(response.getText());
						callback.onSuccess(result);
					} else {
						callback.onFailure(new Exception("Response with error code " + response.getStatusCode() + "\n" + response.getText()));
					}
	...
	public void getAllAddressPaidout(String address, final AsyncCallback<List<AddressPaidoutDTO>> callback) {
		RequestBuilder builder = new RequestBuilder(RequestBuilder.GET, serviceBaseUrl + "waffle/stats/" + address + "/paidout");
		builder.setHeader("Accept", "application/json");
		builder.setHeader("Content-Type", "application/json");
