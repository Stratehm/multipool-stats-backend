...
import strat.mining.multipool.stats.client.mvp.view.coinsolver.PaidoutView;
import strat.mining.multipool.stats.client.mvp.view.coinsolver.impl.PaidoutViewImpl;
...
public class PaidoutViewPresenter implements PaidoutView.PaidoutViewPresenter {
	...
	private void loadData(boolean isAllAddress) {
		...
		// available.
		Map<String, List<AddressPaidoutDTO>> addressesPaidout = clientFactory.getMainDataManager().getCoinsolverDataManager().getDataContainer()
				.getAddressesPaidout();
		if (addressesPaidout.keySet().containsAll(requestedAddresses)) {
			...
			clientFactory.getMainDataManager().getCoinsolverDataManager().loadAllPaidout();
		}
	}
