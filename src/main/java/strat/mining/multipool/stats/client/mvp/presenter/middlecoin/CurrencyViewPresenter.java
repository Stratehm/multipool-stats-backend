...
import strat.mining.multipool.stats.client.mvp.view.middlecoin.impl.CurrencyViewImpl;
...
public class CurrencyViewPresenter implements CurrencyView.CurrencyViewPresenter {
	...
	public CurrencyViewPresenter(ClientFactory clientFactory, final ExchangePlaceDTO exchangePlace, final String currencyCode) {
		...
		clientFactory.getMainDataManager().getMiddlecoinDataManager().addAllTotalStatsLoadHandler(new AllTotalStatsLoadHandler<AddressStatsDTO>() {
		...
		clientFactory.getMainDataManager().getMiddlecoinDataManager().addLastTotalStatsLoadHandler(new LastTotalStatsLoadHandler<AddressStatsDTO>() {
	...
	private void loadAddressesStats() {
		Map<String, List<AddressStatsDTO>> allStats = clientFactory.getMainDataManager().getMiddlecoinDataManager().getDataContainer()
				.getAddressesStats();
		if (clientFactory.getMainDataManager().getMiddlecoinDataManager().getAddresses() != null) {
			for (String address : clientFactory.getMainDataManager().getMiddlecoinDataManager().getAddresses()) {
			...
			List<AddressStatsDTO> totalStats = clientFactory.getMainDataManager().getMiddlecoinDataManager().getDataContainer().getTotalStats();
	...
	public void loadBlockchainInfo() {
		...
		Map<String, BlockchainAddressInfoDTO> blockChainInfo = clientFactory.getMainDataManager().getMiddlecoinDataManager().getDataContainer()
				.getBlockChainInfo();
		if (blockChainInfo != null && blockChainInfo.size() > 0) {
			view.setBlockchainInfo(blockChainInfo);
		} else {
			// If not available, request to load it.
			clientFactory.getMainDataManager().getMiddlecoinDataManager().loadAllBlockchainInfo();
		}

		BlockchainAddressInfoDTO totalBlockchainInfo = clientFactory.getMainDataManager().getMiddlecoinDataManager().getDataContainer()
