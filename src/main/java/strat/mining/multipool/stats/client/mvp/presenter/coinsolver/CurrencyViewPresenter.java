...
import strat.mining.multipool.stats.client.mvp.view.coinsolver.CurrencyView;
import strat.mining.multipool.stats.client.mvp.view.coinsolver.impl.CurrencyViewImpl;
...
public class CurrencyViewPresenter implements CurrencyView.CurrencyViewPresenter {
	...
	public CurrencyViewPresenter(ClientFactory clientFactory, final ExchangePlaceDTO exchangePlace, final String currencyCode) {
		...
		clientFactory.getMainDataManager().getCoinsolverDataManager()
		...
		clientFactory.getMainDataManager().getCoinsolverDataManager().addAllTotalStatsLoadHandler(new AllTotalStatsLoadHandler<AddressStatsDTO>() {
		...
		clientFactory.getMainDataManager().getCoinsolverDataManager().addLastTotalStatsLoadHandler(new LastTotalStatsLoadHandler<AddressStatsDTO>() {
	...
	private void loadAddressesStats() {
		Map<String, List<AddressStatsDTO>> allStats = clientFactory.getMainDataManager().getCoinsolverDataManager().getDataContainer()
				.getAddressesStats();
		if (clientFactory.getMainDataManager().getCoinsolverDataManager().getAddresses() != null) {
			for (String address : clientFactory.getMainDataManager().getCoinsolverDataManager().getAddresses()) {
			...
			List<AddressStatsDTO> totalStats = clientFactory.getMainDataManager().getCoinsolverDataManager().getDataContainer().getTotalStats();
	...
	public void loadBlockchainInfo() {
		...
		Map<String, BlockchainAddressInfoDTO> blockChainInfo = clientFactory.getMainDataManager().getCoinsolverDataManager().getDataContainer()
				.getBlockChainInfo();
		if (blockChainInfo != null && blockChainInfo.size() > 0) {
			view.setBlockchainInfo(blockChainInfo);
		} else {
			// If not available, request to load it.
			clientFactory.getMainDataManager().getCoinsolverDataManager().loadAllBlockchainInfo();
		}

		BlockchainAddressInfoDTO totalBlockchainInfo = clientFactory.getMainDataManager().getCoinsolverDataManager().getDataContainer()
