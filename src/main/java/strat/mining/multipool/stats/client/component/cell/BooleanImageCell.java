...
public class BooleanImageCell extends AbstractCell<Boolean> {
	...
	private ImageResource trueImage;
	private ImageResource falseImage;

	public BooleanImageCell(ImageResource trueImage, ImageResource falseImage) {
	...
	public void render(com.google.gwt.cell.client.Cell.Context context, Boolean value, SafeHtmlBuilder sb) {
		...
		if (value) {
			if (trueImage != null) {
				html += renderer.render(trueImage).asString();
			}
		} else {
			if (falseImage != null) {
				html += renderer.render(falseImage).asString();
			}
		}
		html += "</div>";
