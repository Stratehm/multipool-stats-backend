 ...
 */
package strat.mining.multipool.stats.service.impl.waffle;
...
@Component("waffleAddressSuggestionService")
public class AddressSuggestionService implements SuggestionService<String, String> {
	...
	@PostConstruct
	public void init() {
		LOGGER.info("Initialize WaffleAddressSuggestionService with database addresses.");
		...
		synchronized (LOGGER) {
			...
			LOGGER.debug("Waffle Addresses list update done in {} ms for {} addresses.", System.currentTimeMillis() - startTimeGetAllAddresses,
					currentAddresses.size());
		}

		LOGGER.info("Waffle Initialization done in " + (System.currentTimeMillis() - startTime) + " ms. Nb addresses found: "
				+ currentAddresses.size());
	}
	...
	private void updateAddresses() {
		synchronized (LOGGER) {
			...
			LOGGER.debug("Waffle Addresses list update done in {} ms for {} addresses.", System.currentTimeMillis() - startTime,
					currentAddresses.size());
		}
	}
