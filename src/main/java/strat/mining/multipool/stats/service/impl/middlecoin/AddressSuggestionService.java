...
import strat.mining.multipool.stats.builder.MiddlecoinStatsBuilder;
...
@Component("middlecoinAddressSuggestionService")
public class AddressSuggestionService implements SuggestionService<String, String> {
	...
	@Resource
	private MiddlecoinStatsBuilder statsBuilder;
	...
	@PostConstruct
	public void init() {
		LOGGER.info("Initialize MiddleocinAddressSuggestionService with database addresses.");
		...
		synchronized (LOGGER) {
			...
			LOGGER.debug("Middlecoin Addresses list update done in {} ms for {} addresses.", System.currentTimeMillis() - startTimeGetAllAddresses,
					currentAddresses.size());
		}

		LOGGER.info("Middlecoin Initialization done in " + (System.currentTimeMillis() - startTime) + " ms. Nb addresses found: "
				+ currentAddresses.size());
	}
	...
	private void updateAddresses() {
		synchronized (LOGGER) {
			...
			LOGGER.debug("Middlecoin Addresses list update done in {} ms for {} addresses.", System.currentTimeMillis() - startTime,
					currentAddresses.size());
		}
	}
