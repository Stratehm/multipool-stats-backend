...
@Component("bitstampProvider")
public class BitstampProvider implements ExchangePlaceProvider {

	private static final ExchangePlaceDTO exchangePlaceInfo = new ExchangePlaceDTO();

	@Resource(name = "bitstampCurrencyTickerClient")
	private CurrencyTickerClient bitstampClient;

	public BitstampProvider() {
		exchangePlaceInfo.setLabel("Bitstamp");
		exchangePlaceInfo.setName("bitstamp");
		exchangePlaceInfo.setCurrencyCodes(Lists.newArrayList("USD"));
	...
	public CurrencyTickerDTO getTicker(String currencyCode) {
		return bitstampClient.getCurrencyTicker(currencyCode);
	}
	...
	public List<CurrencyTickerDTO> getTickers() {
		...
		for (String currencyCode : exchangePlaceInfo.getCurrencyCodes()) {
			CurrencyTickerDTO response = bitstampClient.getCurrencyTicker(currencyCode);
			if (response != null) {
				result.add(response);
			}
		}
