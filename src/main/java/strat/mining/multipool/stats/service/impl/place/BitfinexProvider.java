...
@Component("bitfinexProvider")
public class BitfinexProvider implements ExchangePlaceProvider {

	private static final ExchangePlaceDTO exchangePlaceInfo = new ExchangePlaceDTO();

	@Resource(name = "bitfinexCurrencyTickerClient")
	private CurrencyTickerClient bitfinexClient;

	public BitfinexProvider() {
		exchangePlaceInfo.setLabel("Bitfinex");
		exchangePlaceInfo.setName("bitfinex");
	...
	public CurrencyTickerDTO getTicker(String currencyCode) {
		return bitfinexClient.getCurrencyTicker(currencyCode);
	}
	...
	public List<CurrencyTickerDTO> getTickers() {
		...
		for (String currencyCode : exchangePlaceInfo.getCurrencyCodes()) {
			CurrencyTickerDTO response = bitfinexClient.getCurrencyTicker(currencyCode);
			if (response != null) {
				result.add(response);
			}
		}
