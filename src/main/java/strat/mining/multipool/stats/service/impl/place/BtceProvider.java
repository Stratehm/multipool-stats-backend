...
@Component("btceProvider")
public class BtceProvider implements ExchangePlaceProvider {

	private static final ExchangePlaceDTO exchangePlaceInfo = new ExchangePlaceDTO();

	@Resource(name = "btceCurrencyTickerClient")
	private CurrencyTickerClient btceClient;

	public BtceProvider() {
		exchangePlaceInfo.setLabel("BTC-e");
		exchangePlaceInfo.setName("btce");
		exchangePlaceInfo.setCurrencyCodes(Lists.newArrayList("USD", "EUR", "RUR"));
	}
	...
	public CurrencyTickerDTO getTicker(String currencyCode) {
		return btceClient.getCurrencyTicker(currencyCode);
	}
	...
	public List<CurrencyTickerDTO> getTickers() {
		...
		for (String currencyCode : exchangePlaceInfo.getCurrencyCodes()) {
			CurrencyTickerDTO response = btceClient.getCurrencyTicker(currencyCode);
			if (response != null) {
				result.add(response);
			}
		}
