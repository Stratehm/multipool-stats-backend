 ...
 */
package strat.mining.multipool.stats.service.impl.coinshift;
...
@Component("coinshiftAddressSuggestionService")
public class AddressSuggestionService implements SuggestionService<String, String> {
	...
	@PostConstruct
	public void init() {
		LOGGER.info("Initialize coinshiftAddressSuggestionService with database addresses.");
		...
		synchronized (LOGGER) {
			...
			LOGGER.debug("coinshift Addresses list update done in {} ms for {} addresses.", System.currentTimeMillis() - startTimeGetAllAddresses,
					currentAddresses.size());
		}

		LOGGER.info("coinshift Initialization done in " + (System.currentTimeMillis() - startTime) + " ms. Nb addresses found: "
				+ currentAddresses.size());
	}
	...
	private void updateAddresses() {
		synchronized (LOGGER) {
			...
			LOGGER.debug("coinshift Addresses list update done in {} ms for {} addresses.", System.currentTimeMillis() - startTime,
					currentAddresses.size());
		}
	}
